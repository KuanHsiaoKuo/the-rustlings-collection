...
<!-- Added by: runner, at: Sun Feb 26 04:20:02 UTC 2023 -->
...
~~~admonish success title="solution: match -> split_once -> parse::<usize> -> into() or default()" collapsible=true
```rust,editable
...
~~~admonish note title="⚡️try_from_into: with challenge" collapsible=true
```rust,editable
...
~~~admonish success title="solution: essentially return to run the implementation of tuple" collapsible=true
```rust,editable
...
~~~admonish failure title="solution2: Err::Empty -> *Err* is a variant, not a module" collapsible=true
```rust, editable
impl FromStr for Person {
    type Err = ParsePersonError;
    fn from_str(s: &str) -> Result<Person, Self::Err> {
...
~~~admonish failure title="solution3: Err.Empty()" collapsible=true
```rust, editable
impl FromStr for Person {
    type Err = ParsePersonError;
    fn from_str(s: &str) -> Result<Person, Self::Err> {
...
~~~admonish success title="solution4: ParsePersonError::{ErrorType}" collapsible=true
```rust,editable
// Using ParsePersonError::{Empty, BadLen, NoName, ParseInt} above
impl FromStr for Person {
    type Err = ParsePersonError;
